...
async def update_repo(_, message: Message):
    ...
    msg = await message.reply("🔄 `dev : 𝖠𝗆𝗆𝖺𝗋 ...`")
    update_avail = updater()
    if update_avail:
        await msg.edit(" اللتواصل مع المطور لتنصيب بوت  \n\n• X_A_R3 ")
        system("git pull -f && pip3 install -r requirements.txt")
        execle(sys.executable, sys.executable, "main.py", environ)
        return
    await msg.edit("𝖬ყ ** 𝖠𝖼𝖼𝗈𝗎𝗇𝗍**  [- َA𝑚𝑚َ𝑎𝑟 , ُ𝑚𝑜ℎ𝑎𝑚ِ𝑒𝑑 .](https://t.me/X_A_R3)", disable_web_page_preview=True)


@Client.on_message(command(["ريلود", f"restart@{BOT_USERNAME}"]) & ~filters.edited)
@sudo_users_only
