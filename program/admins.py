...
@Client.on_message(
    command(["ايقاف", f"ايقاف@{BOT_USERNAME}", "انهي", f"end@{BOT_USERNAME}", "vstop"])
    & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["كمل", f"كمل@{BOT_USERNAME}", "vresume"]) & other_filters
)
@authorized_users_only
async def resume(client, m: Message):
    ...
    if chat_id in QUEUE:
        ...
        await m.reply("❌ **nothing is currently playing in video call**")
...
async def cbresume(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        ...
        await query.answer("ا", show_alert=True)
...
async def cbstop(_, query: CallbackQuery):
    ...
    if chat_id in QUEUE:
        try:
            await call_py.leave_group_call(chat_id)
            clear_queue(chat_id)
            await query.edit_message_text("✅ **streaming stoped **", reply_markup=bcl)
        except Exception as e:
            await query.edit_message_text(f"🚫 **error:**\n\n`{e}`", reply_markup=bcl)
    else:
        await query.answer("❌ nothing is currently playing ", show_alert=True)
